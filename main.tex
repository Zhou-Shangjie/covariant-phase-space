...
\begin{intu}
    ...
    \end{equation}
    which implies $H_\xi$ is a constant on orbit of $\widetilde{G}$, so $H_\xi$ can induce a function $H_{\xi,\mathcal{P}}$ on $\mathcal{P}$ in a well-defined way and we can define $H_{\xi,\mathcal{P}}(p)=H_{\xi}(q)$ where $\pi(q)=p$.
    Since $\Omega$ is nondegenerate, we can define $\Omega^{-1}:V^\ast(\mathcal{P})\times V^\ast(\mathcal{P})\to \mathbb{R}$ as in \cref{eq:omega-1}.
    \begin{equation}
        X_{\xi,\mathcal{P}}(f)=\Omega^{-1}\left(\delta f,\delta H_{\xi,\mathcal{P}}\right)
    \end{equation}
    where $X_{\xi,\mathcal{P}}\equiv \pi_\ast X_{\xi}$
\end{intu}
